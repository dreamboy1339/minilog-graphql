...
public class MinilogGraphQLExceptionResolver extends DataFetcherExceptionResolverAdapter {
    private static final Logger logger = LoggerFactory.getLogger(MinilogGraphQLExceptionResolver.class);

    @Override
    protected @Nullable GraphQLError resolveToSingleError(Throwable exception, DataFetchingEnvironment env) {
        logger.error("Error occurred while fetching data at {}: {}", env.getExecutionStepInfo().getPath(), exception.getMessage(), exception);
